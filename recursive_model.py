...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        # recursion stream init
        self.h_init = nn.Parameter(torch.zeros(1, 1, config.n_embd))
        self.l_init = nn.Parameter(torch.zeros(1, 1, config.n_embd))
        
        # add small random noise
        nn.init.normal_(self.h_init, mean=0.0, std=0.02)
        nn.init.normal_(self.l_init, mean=0.0, std=0.02)

    def forward(self, idx, targets=None):
        B, T = idx.shape

        # embed tokens (positions embedded via RoPE in attention)
        input_embeddings = self.token_embedding_table(idx)

        z_H = self.h_init.expand(B, T, -1).clone()
        z_L = self.l_init.expand(B, T, -1).clone()
        ...
        with torch.no_grad():
            for _high_step in range(high_loops - 1):
                for _low_step in range(low_loops):
                    z_L = self.reasoning_module(z_L, context=(z_H + input_embeddings))
                z_H = self.reasoning_module(z_H, z_L)
